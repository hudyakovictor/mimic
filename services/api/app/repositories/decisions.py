...
class ReviewRepository(BaseRepository[Review]):
    ...
    async def list_for_decision(self, decision_id: uuid.UUID) -> list[Review]:
        stmt = (
            select(Review)
            .where(Review.decision_id == decision_id)
...
class PhraseTemplateRepository(BaseRepository[PhraseTemplate]):
    ...
    async def list_distinct_words(
        ...
        stmt = (
            select(
                ...
                func.sum(PhraseTemplate.n_samples).label("n_samples"),
                func.max(PhraseTemplate.created_at).label("last_updated"),
                func.max(PhraseTemplate.is_mature).label("is_mature"),
            )
            .where(PhraseTemplate.tenant_id == self.tenant_id)
            .group_by(PhraseTemplate.word, PhraseTemplate.language)
        )
        if language:
        ...
        if cursor:
            ...
            stmt = stmt.having(
                ...
                | (
                    (func.max(PhraseTemplate.created_at) == c["created_at"])
                    & (PhraseTemplate.word < c["id"])
                )
            )
        stmt = stmt.order_by(func.max(PhraseTemplate.created_at).desc(), PhraseTemplate.word.desc()).limit(limit + 1)
        result = await self.session.execute(stmt)
    ...
    async def get(self, template_id: uuid.UUID) -> PhraseTemplate | None:
        return await self.session.get(PhraseTemplate, template_id)
...
class PhraseSampleRepository(BaseRepository[PhraseSample]):
    ...
    async def get(self, sample_id: uuid.UUID) -> PhraseSample | None:
        return await self.session.get(PhraseSample, sample_id)

    async def list_for_word_across_versions(
        self, word: str, language: str, limit: int = 200
    ) -> list[PhraseSample]:
        stmt = (
            select(PhraseSample)
            .where(
