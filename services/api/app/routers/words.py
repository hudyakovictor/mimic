...
async def list_words(
    user: Annotated[object, Depends(require_permission(Permission.BASELINE_READ))],
    ...
    items, _ = await svc.list_words(language, cursor, limit)
    return [
        WordSummary(
            word=x["word"],
            language=x["language"],
...
async def list_templates(
    ...
    svc = WordService(db, s3, user.tenant_id)
    return await svc.list_templates(word, language, cursor, limit)
...
async def list_samples_for_word(
    ...
    svc = WordService(db, s3, user.tenant_id)
    items = await svc.list_samples_for_word(word, language, template_id, limit)
...
async def get_sample_urls(
    ...
    svc = WordService(db, s3, user.tenant_id)
    return await svc.get_sample_urls(sample_id)
